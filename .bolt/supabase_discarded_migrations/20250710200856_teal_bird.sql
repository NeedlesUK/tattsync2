/*
  ...
  1. New Data
    - Adds default tattoo consent form template
    - Adds default piercing consent form template
  2. Template Structure
    - Includes all sections from the HTML form
    - Preserves field structure and validation requirements
*/

-- Only insert if the templates don't already exist
DO $$
BEGIN
  -- Check if we already have templates
  IF NOT EXISTS (SELECT 1 FROM consent_form_templates LIMIT 1) THEN
    ...
    INSERT INTO consent_form_templates (
      ...
      'Default tattoo consent form with medical history and client information',
      true,
      true
    );
    ...
    -- Insert sections for the tattoo template
    ...
    -- Insert fields for the "Your Details" section
    WITH section_id AS (
      SELECT s.id FROM consent_form_template_sections s
      JOIN consent_form_templates t ON s.template_id = t.id
      WHERE t.title = 'Standard Tattoo Consent Form' AND s.title = 'Your Details'
      LIMIT 1
    )
    ...
    -- Insert fields for the "Age & Consent" section
    WITH section_id AS (
      SELECT s.id FROM consent_form_template_sections s
      JOIN consent_form_templates t ON s.template_id = t.id
      WHERE t.title = 'Standard Tattoo Consent Form' AND s.title = 'Age & Consent'
      LIMIT 1
    )
    INSERT INTO consent_form_template_fields (
      section_id,
    ...
    SELECT 
      ...
      'ageConfirm',
      'checkbox',
      'I confirm that I am aged 18 or over and may be asked to produce valid identification (UK Driving Licence or Passport) proving this at my appointment and failure to provide the I.D will result in refusal of service and a charge.',
      NULL,
      true,
      0
    FROM section_id
    ...
    SELECT 
      ...
      'mediaRelease',
      'radio',
      'I release all rights to any photographs and video taken of me and the tattoo and give consent in advance to their reproduction in print or electronic form.',
      ARRAY['Yes', 'No'],
      true,
      3
    FROM section_id
    ...
    -- Insert fields for the "Medical History" section
    WITH section_id AS (
      SELECT s.id FROM consent_form_template_sections s
      JOIN consent_form_templates t ON s.template_id = t.id
      WHERE t.title = 'Standard Tattoo Consent Form' AND s.title = 'Medical History'
      LIMIT 1
    )
    INSERT INTO consent_form_template_fields (
      section_id,
    ...
    SELECT 
      ...
      'noIssues',
      'checkbox',
      'No previous tattoo issues or relevant medical issues',
      NULL,
      false,
      0
    FROM section_id
    ...
    -- Insert fields for the "On The Day" section
    WITH section_id AS (
      SELECT s.id FROM consent_form_template_sections s
      JOIN consent_form_templates t ON s.template_id = t.id
      WHERE t.title = 'Standard Tattoo Consent Form' AND s.title = 'On The Day'
      LIMIT 1
    )
    INSERT INTO consent_form_template_fields (
      section_id,
    ...
    SELECT 
      ...
      'eatBefore',
      'checkbox',
      'l confirm that I have eaten within the 2 hours of the appointment to increase my blood sugar levels.',
      NULL,
      true,
      1
    FROM section_id
    ...
    SELECT 
      ...
      'unwell',
      'checkbox',
      'I understand that if I am unwell or unfit at the time of my appointment that I will inform my artist and my appointment may be cancelled.',
      NULL,
      true,
      2
    FROM section_id
    ...
    SELECT 
      ...
      'noAlcohol',
      'checkbox',
      'I will not get tattooed under the influence of alcohol or drugs.',
      NULL,
      true,
      3
    FROM section_id
    ...
    SELECT 
      ...
      'marketingConsent',
      'radio',
      'I agree for my name and email address to be used by the Event Organiser to inform me of other similar events and partner offers.',
      ARRAY['Yes', 'No'],
      true,
      4
    FROM section_id;
    ...
    INSERT INTO consent_form_templates (
      ...
      'Default piercing consent form with medical history and client information',
      true,
      true
    );
    
    -- Get the ID of the inserted piercing template
    WITH template_id AS (
      SELECT id FROM consent_form_templates 
    ...
    -- Insert sections for the piercing template (similar structure to tattoo template)
    INSERT INTO consent_form_template_sections (
      template_id,
      title,
      description,
    ...
    -- Insert default aftercare templates
