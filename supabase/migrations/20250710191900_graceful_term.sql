...
-- Create function to create aftercare templates table if it doesn't exist
CREATE OR REPLACE FUNCTION create_aftercare_templates_table()
RETURNS void AS $$
BEGIN
  -- This function is a no-op since we've already created the table above
  -- It's here for compatibility with the frontend code that calls it
END;
$$ LANGUAGE plpgsql;