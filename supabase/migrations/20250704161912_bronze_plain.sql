/*
  # Profile Visibility and User Roles

  1. New Features
    - Add show_profile column to user_profiles table
    - Create function to check profile visibility based on context
    - Implement user roles system with primary role support

  2. Security
    - Ensure profiles are only visible to authorized users
    - Allow event managers to view profiles of their event participants
    - Allow studio members to view profiles of their colleagues
    - Allow clients to view profiles of artists they've booked

  3. Changes
    - Add show_profile column to user_profiles table
    - Create check_profile_visibility function
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Created user_roles table and migrated existing roles';
  ELSE
    RAISE NOTICE 'user_roles table already exists';
  END IF;
END $$;

-- Create function to get all roles for a user if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Created get_user_roles function';
  ELSE
    RAISE NOTICE 'get_user_roles function already exists';
  END IF;
END $$;

-- Create function to set primary role if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    ...
    CREATE FUNCTION set_primary_role(user_uuid UUID, primary_role user_role)
    RETURNS BOOLEAN AS $$
    DECLARE
      role_exists BOOLEAN;
    BEGIN
    ...
    RAISE NOTICE 'Created set_primary_role function';
  ELSE
    RAISE NOTICE 'set_primary_role function already exists';
  END IF;
END $$;

-- Create function to add a role to a user if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    ...
    CREATE FUNCTION add_user_role(user_uuid UUID, new_role user_role)
    RETURNS BOOLEAN AS $$
    BEGIN
      INSERT INTO user_roles (user_id, role, is_primary)
    ...
    RAISE NOTICE 'Created add_user_role function';
  ELSE
    RAISE NOTICE 'add_user_role function already exists';
  END IF;
END $$;

-- Create function to remove a role from a user if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    ...
    CREATE FUNCTION remove_user_role(user_uuid UUID, role_to_remove user_role)
    RETURNS BOOLEAN AS $$
    DECLARE
      is_primary BOOLEAN;
      role_count INTEGER;
    ...
    RAISE NOTICE 'Created remove_user_role function';
  ELSE
    RAISE NOTICE 'remove_user_role function already exists';
  END IF;
END $$;
