/*
  ...
  1. New Tables
    - `user_roles` - Junction table to store multiple roles per user
    - Updates to user_profiles table to support multiple roles
  ...
  3. Changes
    - Update existing users to have their current role in the junction table
...
-- Verify the admin has all roles
DO $$
DECLARE
  admin_id uuid;
  role_count integer;
...
BEGIN
  ...
  -- Skip if admin not found
  IF admin_id IS NULL THEN
    RAISE NOTICE 'Admin user not found, skipping verification';
    RETURN;
  END IF;
  
  -- Count roles for admin
  SELECT COUNT(*) INTO role_count
  FROM user_roles
  ...
  FROM (SELECT unnest(enum_range(NULL::user_role))) as roles;
  
  -- Output results
  RAISE NOTICE 'Admin has % roles out of % possible roles', role_count, expected_count;
  
  IF role_count = expected_count THEN
    RAISE NOTICE 'Admin has all possible roles';
  ELSE
    RAISE WARNING 'Admin is missing some roles';
  END IF;
END
$$;