/*
  # User Roles and Profiles Migration
  ...
  3. Security
    ...
    - Admin access to all profiles and roles
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can manage all roles"
      ON user_roles
      FOR ALL
      TO authenticated
      USING (
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Users can manage their own profiles"
      ON user_profiles
      FOR ALL
      TO authenticated
      USING (user_id = auth.uid())';
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can manage all profiles"
      ON user_profiles
      FOR ALL
      TO authenticated
      USING (
